package br.com.franca.helpdesk.util;

import br.com.franca.helpdesk.domains.enums.Perfil;
import com.fasterxml.jackson.core.JacksonException;
import com.fasterxml.jackson.core.JsonParser;
import com.fasterxml.jackson.databind.DeserializationContext;
import com.fasterxml.jackson.databind.JsonDeserializer;
import java.io.IOException;

public class PerfilDeserializer extends JsonDeserializer<Perfil> {


    @Override
    public Perfil deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JacksonException {
        int code = jsonParser.getIntValue();
        return Perfil.toEnum(code);
    }
}